...
I already was using two methods and one for the initial picks submission and one for changes in the [PlayerPicks service][2]. If the player was changing their picks, I used an if / else statement that compared the current time to when the season started:
...
The multiplier is reset to 1 and if their original pick had qualified for the [double points bonus][3], it needs to reset back to one as the UniquePicks service is re-run after the All-Star Break and then will assign a multiplier of 2 if the new pick qualifies for the double points bonus.
